        ...
        <div class="px-4 py-3 bg-white rounded-lg shadow-md">
            ...
            >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach (['fb' => 'Facebook', 'ig' => 'Instagram', 'tiktok' => 'TikTok', 'yt' => 'YouTube'] as $field => $label)
                        <div>
                            ...
                            <input name="{{ $field }}" type="text" value="{{ old($field, $identity->$field ?? '') }}"
                ...
                <div>
                    ...
                    <input name="time_service" type="text" value="{{ old('service', $identity->time_service ?? '') }}"
                    ...
                    @foreach ([1, 2] as $i)
                        <div>
                            ...
                            <label for="dropzone-file{{ $i }}"
                                ...
                                <input id="dropzone-file{{ $i }}" type="file" class="hidden" name="image{{ $i }}" accept="image/*" onchange="previewImage(event, 'preview-image{{ $i }}')" />
                            </label>
                        </div>
                    @endforeach
                </div>

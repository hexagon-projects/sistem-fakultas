<x-layout title="Add Jurnal">
    ...
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('jurnal.update', $jurnal->id) }}" method="post" enctype="multipart/form-data" id="form-departement">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ old('title', $jurnal->title ?? '') }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="name" value="{{ old('name', $jurnal->name ?? '') }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('title')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $jurnal->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview"
                                src="{{ isset($jurnal) && $jurnal->image1 ? asset('storage/' . $jurnal->image1) : '#' }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ isset($jurnal) && $jurnal->image1 ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            ...
                            <button
                                ...
                                class="{{ isset($jurnal) && $jurnal->image1 ? '' : 'hidden' }} absolute top-1 right-1 bg-red-600 text-white text-xs px-2 py-1 rounded z-20"
                            >
                                Hapus
                            </button>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                            id="image-preview-2"
                            src="{{ isset($jurnal) && $jurnal->image2 ? asset('storage/' . $jurnal->image2) : '#' }}" 
                            alt="Preview" 
                            class="absolute inset-0 w-full h-full object-cover {{ isset($jurnal) && $jurnal->image2 ? '' : 'hidden' }} z-0 rounded-md" 
                        />
                            ...
                            <button
                                ...
                                onclick="removeImage2()"
                                 class="{{ isset($jurnal) && $jurnal->image2 ? '' : 'hidden' }} absolute top-1 right-1 bg-red-600 text-white text-xs px-2 py-1 rounded z-20"
                            >
                                Hapus
                            </button>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                            id="image-preview-2"
                            src="{{ isset($jurnal) && $jurnal->image3 ? asset('storage/' . $jurnal->image3) : '#' }}" 
                            alt="Preview" 
                            class="absolute inset-0 w-full h-full object-cover {{ isset($jurnal) && $jurnal->image3 ? '' : 'hidden' }} z-0 rounded-md" 
                        />
                            ...
                            <button
                                ...
                                onclick="removeImage3()"
                                 class="{{ isset($jurnal) && $jurnal->image3 ? '' : 'hidden' }} absolute top-1 right-1 bg-red-600 text-white text-xs px-2 py-1 rounded z-20"
                            >
                                Hapus
                            </button>
                    ...
                    <select name="id_team" class="select2 w-full" id="id_team">
                        ...
                        @foreach ($teams as $team)
                            <option value="{{ $team->id }}" @selected(old('id_team', $jurnal->id_team ?? '') == $team->id)>{{ $team->name }}</option>
                        @endforeach
                    </select>

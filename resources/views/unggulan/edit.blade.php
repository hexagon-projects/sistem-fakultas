<x-layout title="USP">
    ...
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('unggulan.update', $unggulan->id) }}" method="post" enctype="multipart/form-data" id="form-unggulan">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="id_departement">unggulan</label>
                        <select name="id_departement" id="id_departement"
                            ...
                            @if ($unggulan->id_departement == $departement->id)
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ $unggulan->title }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $unggulan->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                        ...
                        <img 
                            id="image-preview" 
                            src="{{ old('image') ? asset('storage/usp/' . old('image')) : (isset($unggulan) && $unggulan->image ? asset('storage/' . $unggulan->image) : '#') }}" 
                            alt="Preview" 
                            class="absolute inset-0 w-full h-full object-cover {{ (isset($unggulan) && $unggulan->image) ? '' : 'hidden' }} z-0 rounded-md" 
                        />
                        ...
                        <!-- Text Default -->
                        <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($unggulan) && $unggulan->image) ? 'hidden' : '' }}">
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('unggulan.index') }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>
    ...
    function confirmSubmit() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('form-unggulan').submit();
            }
        });
    }
</script>

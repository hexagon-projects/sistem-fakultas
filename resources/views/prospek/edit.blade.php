<x-layout title="Prospek">
    ...
    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('prospek.update', $prospek->id) }}" method="post" enctype="multipart/form-data" id="form-prospek">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="id_departement">prospek</label>
                        <select name="id_departement" id="id_departement"
                            ...
                            @if ($prospek->id_departement == $departement->id)
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ $prospek->title }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $prospek->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="flex flex-col gap-1 mb-5">
                    ...
                    <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                        ...
                        <img 
                            id="image-preview" 
                            src="{{ old('image') ? asset('storage/usp/' . old('image')) : (isset($prospek) && $prospek->image ? asset('storage/' . $prospek->image) : '#') }}" 
                            alt="Preview" 
                            class="absolute inset-0 w-full h-full object-cover {{ (isset($prospek) && $prospek->image) ? '' : 'hidden' }} z-0 rounded-md" 
                        />
                        ...
                        <!-- Text Default -->
                        <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($prospek) && $prospek->image) ? 'hidden' : '' }}">
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img
                                id="icon-preview"
                                src="{{ old('icon') ? asset('storage/usp/' . old('icon')) : (isset($prospek) && $prospek->icon ? asset('storage/' . $prospek->icon) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ (isset($prospek) && $prospek->icon) ? '' : 'hidden' }} z-0 rounded-md" 
                            />
    ...
    function confirmSubmit() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('form-prospek').submit();
            }
        });
    }
</script>

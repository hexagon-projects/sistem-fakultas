<x-layout title="Update Archievment">

    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('achievement.update', $achievement->id) }}" method="post" enctype="multipart/form-data" id="form-departement">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="home" id="home" value="1"
                                {{ old('home', $achievement->home) ? 'checked' : '' }}
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="id_departement" id="id_departement"
                            ...
                            @if ($achievement->id_departement == $departement->id)
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="category" id="category"
                            ...
                            <option value="regional" {{ old('category', $achievement->category ?? '') == 'regional' ? 'selected' : '' }}>Regional</option>
                            <option value="nasional" {{ old('category', $achievement->category ?? '') == 'nasional' ? 'selected' : '' }}>Nasional</option>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $achievement->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview" 
                                src="{{ old('image') ? asset('storage/achievements/' . old('image')) : (isset($achievement) && $achievement->image ? asset('storage/' . $achievement->image) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ (isset($achievement) && $achievement->image) ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            ...
                            <!-- Text Default -->
                            <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($achievement) && $achievement->image) ? 'hidden' : '' }}">
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('achievement.index') }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>

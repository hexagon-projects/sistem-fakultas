<x-layout title="Timeline">

    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('timeline.update', $timeline->id) }}" method="post" enctype="multipart/form-data" id="form-departement">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="home" id="home" value="1"
                                {{ old('home', $timeline->home) ? 'checked' : '' }}
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ old('title', $timeline->title) }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $timeline->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                            id="image-preview" 
                            src="{{ old('image') ? asset('storage/timeline-image/' . old('image')) : (isset($timeline) && $timeline->image ? asset('storage/' . $timeline->image) : '#') }}" 
                            alt="Preview" 
                            class="absolute inset-0 w-full h-full object-cover {{ (isset($timeline) && $timeline->image) ? '' : 'hidden' }} z-0 rounded-md" 
                        />
                        ...
                        <div class="flex flex-col gap-1 mb-5">
                            ...
                            <input type="number" name="no_urut" value="{{ old('no_urut', $timeline->no_urut) }}"
                                class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                            @error('no_urut')
                                <div>
                                    <small class="text-red-500"><i>{{ $message }}</i></small>
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('timeline.index') }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>

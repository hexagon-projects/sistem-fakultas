<x-layout title="Add Testimoni">

    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('testimoni.update', $testimoni->id) }}" method="post" enctype="multipart/form-data" id="form-departement">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    <div class="flex flex-col gap-1 mb-5 md:col-span-2">
                        <div class="flex items-center gap-2">
                            ...
                            {{ old('home', $testimoni->home) ? 'checked' : '' }}
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="id_departement" id="id_departement"
                            ...
                            @if ($testimoni->id_departement == $departement->id)
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ $testimoni->title }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('title')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <textarea name="description" id="description" class="block w-full mt-1 text-sm border-gray-300 border-2 p-2 rounded-md">{{ $testimoni->description }}</textarea>
                        @error('description')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                            </div>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview" 
                                src="{{ old('image') ? asset('storage/testimonials/' . old('image')) : (isset($testimoni) && $testimoni->image ? asset('storage/' . $testimoni->image) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ (isset($testimoni) && $testimoni->image) ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            ...
                            <!-- Text Default -->
                            <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($testimoni) && $testimoni->image) ? 'hidden' : '' }}">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="yt" value="{{ $testimoni->yt }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('yt')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('testimoni.index') }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>

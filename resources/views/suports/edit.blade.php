<x-layout title="Add Suport">

    <div class="p-5 flex items-center justify-center">
        <div class="w-full bg-white p-5 rounded-xl text-sm flex flex-col">
            <form action="{{ route('suport.update', $suport->id) }}" method="post" enctype="multipart/form-data" id="form-departement">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 md:col-span-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <select name="id_departement" id="id_departement"
                            ...
                            @if ($suport->id_departement == $departement->id)
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="title" value="{{ $suport->title }}"
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="name" value="{{ $suport->name }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('name')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        <label for="support_by">Suport By</label>
                        <input type="text" name="support_by" value="{{ $suport->support_by }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('support_by')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
                ...
                <div class="col-span-1">
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <div class="relative w-full h-40 border-2 border-dashed rounded-md flex items-center justify-center bg-gray-50 overflow-hidden">
                            ...
                            <img 
                                id="image-preview" 
                                src="{{ old('image') ? asset('storage/supports/' . old('image')) : (isset($suport) && $suport->image ? asset('storage/' . $suport->image) : '#') }}" 
                                alt="Preview" 
                                class="absolute inset-0 w-full h-full object-cover {{ (isset($suport) && $suport->image) ? '' : 'hidden' }} z-0 rounded-md" 
                            />
                            ...
                            <!-- Text Default -->
                            <span id="upload-text" class="text-gray-400 z-0 text-center px-2 {{ (isset($suport) && $suport->image) ? 'hidden' : '' }}">
                    ...
                    <div class="flex flex-col gap-1 mb-5">
                        ...
                        <input type="text" name="yt" value="{{ $suport->yt }}"
                            class="block w-full mt-1 text-sm focus:border-[#5676ff] focus:outline-none focus:shadow-outline-purple form-input border-gray-300 border-2 p-2 rounded-md" />
                        @error('yt')
                            <div>
                                <small class="text-red-500"><i>{{ $message }}</i></small>
            ...
            <div class="flex items-center justify-between my-5">
                <a href="{{ route('suport.index') }}" class="py-3 px-5 bg-gray-300 rounded-xl">Back</a>
